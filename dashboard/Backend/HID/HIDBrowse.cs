...
namespace Mighty.HID
{
    public class HIDBrowse
    {
        ...
        public static List<IDeviceInfo> Browse()
        {
            try
            {
                ...
                List<IDeviceInfo> info = new List<IDeviceInfo>();
                    ...
                    index, ref iface))
                {
                    ...
                    if (handle != Native.INVALID_HANDLE_VALUE)
                    {
                        ...
                        /* get usagepage */
                        IntPtr preparedData;
                        Native.HidD_GetPreparsedData(handle, out preparedData);
                        Native.HidCaps hidcaps = new Native.HidCaps();
                        Native.HidP_GetCaps(preparedData, out hidcaps);
                        /* build up a new element */
                        IDeviceInfo i = new HIDInfo(prod, serial, man, path, vid, pid, string.Format("{0:X4}", hidcaps.UsagePage));
                        /* add to list */
                        info.Add(i);
