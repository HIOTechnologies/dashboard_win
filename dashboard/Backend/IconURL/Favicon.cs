...
namespace HIO.Backend.IconURL
{
    ...
    public class Favicon
    {
        ...
        public static Favicon GetFromUrl(string url, int timeout = DEFAULT_TIMEOUT)
        {
            try
            {
                return GetFromUrl(new Uri("http://"+url), timeout);
            }
            catch {

                return GetFromUrl(new Uri("https://" + url), timeout);
        ...
        private string ExtractFavIconUrl(Uri url)
        {
            ...
            // Link
            foreach (Match m in Regex.Matches(html, "<link[^>]*(rel=\"icon\"|rel=\"shortcut icon\")[^>]*[\\/]?>", RegexOptions.IgnoreCase)) /*|rel=\"apple-touch-icon\"|rel=\"apple-touch-icon-precomposed\"*/
            {
